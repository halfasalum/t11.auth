...
class Dashboard extends BaseController
{
    public function officer_dashboard()
    {
        ...
        $user_branch = $this->getUserBranches();
    ...
    public function collectionTrend($zones = [], $branches = [])
    {
        ...
        if (in_array(21, $controls)) {
            ...
            $payments = PaymentSubmissions::selectRaw('payment_due_date, SUM(amount) as total_paid')
                ->join('loan_payment_schedule', 'payment_submissions.schedule_id', '=', 'loan_payment_schedule.id')
                ->where('payment_submissions.company', $user_company)
                ->where('payment_submissions.submission_status', 11)
                ->whereIn('payment_due_date', $dates)
                ->whereIn('loan_payment_schedule.zone', $zones)
                ->groupBy('payment_due_date')
                ->get()
                ->keyBy('payment_due_date');

            $results = $trends->map(function ($item) use ($payments) {
                $paymentDate = Carbon::parse($item->payment_due_date)->format('Y-m-d');
                $collectedAmount = $payments->has($paymentDate) ? $payments->get($paymentDate)->total_paid : 0;
                return [
                    'date' => $paymentDate,
