...
class Loanss extends Model
{
    ...
    public function getStatusColorAttribute(): string
    {
        return match ($this->status) {
            ...
            self::STATUS_FORECLOSURE => 'danger',
            default => 'secondary',
        };
    }
    ...
    {
        return $this->product()->first()->shouldBeDefaulted($this);
    }

    public function shouldBeWrittenOff(): bool
    {
        return $this->product()->first()->shouldBeWrittenOff($this);
    }

    public function shouldBeForeclosed(): bool
    {
        return $this->product()->first()->shouldBeForeclosed($this);
    }

    public function getRequiredApprovalLevel($action): string
    {
        return $this->product()->first()->getApprovalLevelForAction($action);
    }
}
